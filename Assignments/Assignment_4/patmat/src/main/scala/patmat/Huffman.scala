...
object Huffman
{
  ...
  def weight(tree: CodeTree): Int = tree match
  {
    ...
    case _ => throw new NotImplementedError( "weight()" )
  }
  ...
  def chars(tree: CodeTree): List[Char] = tree match
  {
    ...
    case f: Fork => f.chars
    case _ => throw new NotImplementedError( "chars()" )
  }
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
  {
    ...
    var freqList = List[Int ]()
    for (c <- chars)
    {
      //val elem = list.find( _._1 == c )
      val i = charList.indexOf(c)
      if ( 0 <= i ) { freqList.patch(i, Seq(freqList(i)+1), 1) }
      else { charList = c :: charList ; freqList = 1 :: freqList }
    }
    charList zip freqList
    */
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
  {
    ...
    /*
    var leafList: List[Leaf] = Nil
    var freqList = freqs
    while ( !(freqList.isEmpty) )
    {
      var lowestFreq = Int.MaxValue
      var lowestFreqIndex = -1
      var elem = (' ', -1)
      var leaf = Leaf( ' ', -1 )
      var i = 0
      for ((c, f) <- freqList)
      {
        if ( f < lowestFreq ) { lowestFreq = f ; elem = (c, f) ; leaf = Leaf(c, f) }
      }
      if ( leafList.isEmpty ) { leafList = List(leaf) }
      else { leafList = leafList :+ leaf }
      freqList = freqList diff List(elem)
    }
    leafList
    */
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = //trees match
  {
    ...
    case _ => trees
    */
    if ( 2 > trees.length ) { trees }
    else
    {
      val leftLeaf  = trees(0)
      val rightLeaf = trees(1)
      ( makeCodeTree(leftLeaf, rightLeaf) :: trees.tail.tail ).sortBy(t => weight(t))
    }
  }
  ...
  def until(isSingleton: List[CodeTree] => Boolean,
            merge: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): CodeTree =
  {
    if ( isSingleton(trees) ) { trees(0) }
    else until( isSingleton, merge )(merge(trees))
  }
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    {
      ...
      traverse(tree, bits)
      */
      def decode0(subtree: CodeTree, bits: List[Bit]): List[Char] = subtree match
      {
        case l: Leaf =>
          if ( bits.isEmpty ) { List(l.char) }
          else { l.char :: decode0(tree, bits) }   //  start to decode next character
        case f: Fork =>
          if (0 == bits.head) { decode0(f.left, bits.tail) }
          else { decode0(f.right, bits.tail) }
      }
      decode0(tree, bits)
    }
      ...
      */
      ...
      for ( c <- text )
      {
        ...
        while ( !isDone )
        {
          subtree match
          {
            case l: Leaf => isDone = true
            case f: Fork =>
            {
              var isLeft = false
              f.left match
              {
                case l: Leaf => isLeft = (l.char == c)
                case f: Fork => isLeft = f.chars.contains(c)
              }
              if ( isLeft ) { list = list :+ 0 ; subtree = f.left }
              else { list = list :+ 1 ; subtree = f.right }
            }
          }
        }
        subtree = tree
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] =
    {
      ...
      val index = table.indexWhere(v => v._1 == char)
      val pair: (Char, List[Bit]) = table( index )
      pair._2
    }
